<?php
	...
	if(isset($_SESSION['user'])){
		try{
			...
			$stmt = $conn->prepare("SELECT chatbox.*,users.firstname,users.photo as profile_picture FROM job_order JOIN chatbox ON job_order.id = chatbox.order_id JOIN users ON job_order.emp_assigned_id = users.id where job_order.id =:job_id order by chatbox.created_date ASC");
